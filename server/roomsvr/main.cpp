/*
 * * file name: main.cpp
 * * description: roomsvr进程入口。支持两种启动模式：
 *
 *   模式1（命令行）：
 *     roomsvr <svr_id> <tbus2_agent_url>
 *       svr_id:          本进程实例编号，支持0x前缀十六进制，实际busid=0x06000000|svr_id
 *       tbus2_agent_url: 本地tbus2 agent地址，如"tcp://127.0.0.1:8001"
 *
 *   模式2（TCM --conf-file）：
 *     roomsvr --conf-file=<path>
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        svr_id = roomsvr::RoomApp::kRoomGroupBase | raw_id;
    }

    roomsvr::RoomApp::GetInst().Setup(tbus2_agent_url);

    return app::mainhelper::RunApp(roomsvr::RoomApp::GetInst(), argc, argv, svr_id, "RoomApp");
}

...
namespace doc {
  ...
  public:
    ...
    // It's not safe to print the buffer returned by rawData() since StringData
    // could wrap a non null-terminated string. This method always returns a 
    // null-terminated string if user provides a non-null buf.
    const char* nullTerminatedString(char* buf, size_t len) const
    {
        if (!buf || !_data || len == 0)
        { 
            return ""; 

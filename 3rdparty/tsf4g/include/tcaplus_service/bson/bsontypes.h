...
namespace doc {
    ...
    */
    ...
    inline int canonicalizeBSONType(BSONType type) {
        ...
        case CodeWScope:
            return 65;
        default:
			msgasserted(10014, "canonicalizeBSONType(BSONType type) error,reason:type not defined"); 
            return -1;
        }
    }
    ...
    */
    ...
    /**
     * @brief the space occupancy of every built-in fixed field in the BSON objects
     */
    enum BO_BUILTIN_FIELD_SPACE
    {
        // the order of every field in the header of a BSONObj
        BO_HEADER_TOTAL = 8, // the total size of a BO_HEADER
        BO_HEADER_OBJ_SIZE = 4, // indicates the size of a BSONObj
        BO_HEADER_SIMPLE_CHILD_NUM = 4, // indicates the number of simple children of an obj/array 
    
        BO_ELEMENT_TYPE = 1, // field in the BSONElement that indicates the type of an element

        BO_EOO_TYPE = 1, // size of EOO type
    }; // enum BO_BUILTIN_FIELD_SPACE

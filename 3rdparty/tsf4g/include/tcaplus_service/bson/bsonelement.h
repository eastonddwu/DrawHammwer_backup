...
/**
 * @brief since we've modified the internal implementation of BSON
 *      this data interchange format (DIF) may be not compatible backwards
 *      so a macro is used to declare the current version
 */
#define TCAPLUS_BSON_BUILD_VERSION 0X00000010
...
namespace doc {
    ...
    */
    ...
    public:
        ...
        */
        std::string String()        const { return chk(tcaplus::doc::String).str(); }
        Date_t Date()               const { return chk(tcaplus::doc::Date).date(); }
        ...
        { 
            switch (type())
            {
            case NumberInt:  return _numberInt();
            case NumberLong: return _numberLong();
            // FIXME: throw an exception or pass out an error code to explicitly indicate type mismatching
            default:         return 0;
            }
        }
        int Int()                   const { return chk(NumberInt)._numberInt(); }
        bool Bool()                 const { return chk(tcaplus::doc::Bool).boolean(); }
        std::vector<BSONElement> Array() const; // see implementation for detailed comments
        tcaplus::doc::OID OID()            const { return chk(jstOID).__oid(); }
        ...
        */
        ...
        void Val(tcaplus::doc::OID& v)     const { v = OID(); }
        ...
        */
        ...
        const char * value() const {
            return (data + fieldNameSize() + BO_ELEMENT_TYPE);
        }
        /** size in bytes of the element's value (when applicable). */
        int valuesize() const {
            return size() - fieldNameSize() - BO_ELEMENT_TYPE;
        }

        bool isBoolean() const { return type() == tcaplus::doc::Bool; }
        ...
        */
        ...
        const tcaplus::doc::OID &__oid() const { return *reinterpret_cast< const tcaplus::doc::OID* >(value()); }
        ...
        */
        ...
        const char *valuestrsafe() const {
            return type() == tcaplus::doc::String ? valuestr() : "";
        }
        /** Get the string value of the element.  If not a string returns "". */
        std::string str() const {
            return type() == tcaplus::doc::String ? std::string(valuestr(), valuestrsize()-1) : std::string();
        }
        ...
        const char *binData(int& len) const {
			...
			if(!( type() == BinData ))
			{
				msgasserted(10006, "binData(int& len) error,reason:(!( type() == BinData ))"); 
			}
        ...
        BinDataType binDataType() const {
            // BinData: <int len> <byte subtype> <byte[len] data>
 			if(!( type() == BinData ))
			{
				msgasserted(10006, "binDataType() error,reason:(!( type() == BinData ))"); 
			}
        ...
        const char *regex() const {
 			if(!( type() == RegEx ))
			{
				msgasserted(10007, "regex() error,reason:(!( type() == RegEx ))"); 
			}
        ...
        */
        ...
        const char * dbrefNS() const {
			if(!(type() == DBRef))
			{
				msgasserted(10063, "dbrefNS() error,reason:(!(type() == DBRef))"); 
			}

            return value() + 4;
        }

        const tcaplus::doc::OID& dbrefOID() const {
			if(!(type() == DBRef))
			{
				msgasserted(10064, "dbrefOID() error,reason:(!(type() == DBRef))"); 
			}

			
            const char * start = value();
            start += 4 + *reinterpret_cast< const int* >( start );  // todo endian
            return *reinterpret_cast< const tcaplus::doc::OID* >( start );
        }
        ...
        explicit BSONElement(const char *d, int maxLen) : data(d) {
            if ( eoo() ) {
                ...
                if ( maxLen != -1 ) {
					...
					if(!(size != -1))
					{
						msgasserted(10333, "BSONElement(const char *d, int maxLen) : data(d) error,reason:(!(size != -1))"); 
					}
    ...
    inline bool BSONElement::trueValue() const {
        ...
        case NumberDouble:
			{
				if(_numberDouble() >= -EPSINON && _numberDouble() <= EPSINON )
				{
					return false;
				}
				else
    ...
    inline bool BSONElement::isSimpleType() const {
        ...
        case NumberDouble:
        case NumberInt:
        case tcaplus::doc::String:
        case tcaplus::doc::Bool:
        case tcaplus::doc::Date:
        case jstOID:
            return true;
        default:
            return false;
    ...
    */
    inline BSONElement::BSONElement() {
        static char z = static_cast<char>(EOO);
        data = &z;
        fieldNameSize_ = 0;
        totalSize = 1;
    }

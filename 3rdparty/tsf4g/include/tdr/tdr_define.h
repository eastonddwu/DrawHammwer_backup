...
*  @{
*/
enum tagTDRInputFlag
{
	/**
	默认的输入处理标记，带有此标记的API处理行为和之前版本一致，这里主要用于区分，
	增加了TDR_INPUT_FLAG_SET_DEFAULT或其标记的行为
	*/
	TDR_INPUT_FLAG_NONE = 0x00,

	/**
	在2.7.37版本及之前版本中，部分input处理的API不会特殊处理输入中缺失的成员，
	支持此标记支持的input函数，将会把它们设置为默认值
	*/
	TDR_INPUT_FLAG_SET_DEFAULT = 0x01,
};
/** @} */

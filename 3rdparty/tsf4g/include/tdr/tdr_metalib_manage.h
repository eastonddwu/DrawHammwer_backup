...
#define tdr_sizeinfo_to_off_i   tdr_sizeinfo_to_off
    ...
    /**从unionid获取entryidx，通常与tdr_unpack_union_entry_v2配合使用
     *@param[in] a_pstIndex index 结构的指针
     *
     *@return union id所对应的entry是第几个
     *
     *@pre \e a_pstMeta 不能为 NULL
     */
    TDR_API int tdr_get_entryidx_by_unionid(IN LPTDRMETA a_pstMeta, IN int a_iID);
    #if 0

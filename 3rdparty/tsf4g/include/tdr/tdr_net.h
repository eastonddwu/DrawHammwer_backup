...
/**
*按照指定版本根据元数据描述将网络交换消息转换成本地存储数据
*tdr_ntoh当参数为(v2版本meta,v1版本结构体,v2版本打包buf,v1版本version)时会无法解包,所以新添了tdr_ntoh_v2接口，用以反向兼容
*@param[in] a_pstHostMeta	a_pstHost对应的元数据描述的指针
*@param[in,out]	a_pstHost 存储本地结构信息的结构体指针
*@param[in]	a_pstNetMeta 网络打包时的元数据描述的指针
*@param[in,out]	a_pstNet 存储网络信息结构体指针
*@param[in] a_iVersion	指定编码在网络信息块中结构体的版本号
*
*@note 新增a_pstHostMeta参数的作用：
	*	a_pstHostMeta为NULL，等同于调用tdr_ntoh函数
	*	a_pstHostMeta不为NULL，指定参数a_pstHost对应的元数据描述的指针，解包时：
        *   对于大于a_pstHostMeta版本号的字段，会跳过处理并且不会移动解包指针(修改a_pstHost内容的指针)
        *   对于有refer字段且调大过数组长度的字段，若是a_pstHostMeta数组长度最大值小于refer所指，会进行裁剪处理 
*@note 该接口涉及到一个全局buffer，v2解包时会申请一块内存来做数据存储，为了避免内存脆片，当tdr_get_meta_size(netMeta)比该内存已有的长度大时，才
*      会释放掉该内存并重新申请。全局变量为g_pstV2Buffer。此外还可以调用tdr_free_v2buffer来释放该buffer，tdr_get_v2buffer_len来获取该内存大小。
*@note 如果a_iVersion为0或大于a_pstHostMeta的最高版本(tdr_get_meta_current_version)，a_pstHostMeta在解包时会按照a_pstHostMeta的最高版本来裁剪
* @pre \e a_pstHostMeta 不为NULL时，必须对应 xml 描述文件中 struct 元素。且其name要等于a_pstNetMeta的name
* @pre \e a_pstNetMeta 不能为 NULL，必须对应 xml 描述文件中 struct 元素。
* @pre \e a_pstHostMeta描述的最高版本 <= a_pstMeta描述的最高版本
* @pre \e a_pstHost 不能为NULL
* @pre \e a_pstHost.pszBuff 不能为 NULL
* @pre \e a_pstHost.iBuff 必须大于0
* @pre \e a_pstNet 不能为 NULL
...
* @note 如果该接口返回失败，可以调用 tdr_get_error_detail 取得详细的错误信息
*
* @see tdr_free_v2buffer
* @see tdr_get_v2buffer_len
*/
TDR_API int tdr_ntoh_v2(IN LPTDRMETA a_pstHostMeta, INOUT LPTDRDATA a_pstHost, IN LPTDRMETA a_pstNetMeta, INOUT LPTDRDATA a_pstNet, IN int a_iVersion);
/**当Meta为union类型时v2版本的解包
 *@param[in] a_entryId 当前entry对应的id
*/
TDR_API int tdr_unpack_union_entry_v2(IN LPTDRMETA a_pstHostMeta, INOUT LPTDRDATA a_pstHostData,
                              IN LPTDRMETA a_pstNetMeta, INOUT LPTDRDATA a_pstNetData, IN int a_entryId, IN int a_iVersion);

/** 释放掉v2解包时申请的全局buffer
*/
TDR_API void tdr_free_v2buffer();

/** 获取v2解包时申请的全局buffer长度
*/
TDR_API int tdr_get_v2buffer_len();

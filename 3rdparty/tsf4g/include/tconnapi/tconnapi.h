...
#define GUID2IDX_SHIFT 29 /* 多对一部署，GUID转换到idx时的移位距离 */
...
// 多对一部署中，获取通道序号，与tconnapi_connect调用顺序相关
#define GET_SHIFTED_GUID_MASK(ConnIdx) \
  (((ConnIdx) & (~DEL_GUID_MASK)) >> GUID2IDX_SHIFT)

/**
 *@brief
 *使用tconnapi之前先必须调用该函数，如果使用动态tbus模式，请调用tconnd_init_with_automatic_tbus
 *@brief 初始化API信息
...
/**
 *@brief
 *使用tconnapi之前先必须调用该函数，动态tbus模式专用，如果是普通模式，请使用tconnapi_init
 *@brief 初始化tbus相关信息
 ...
 *@retval   <0:fail
 */
int tconnapi_init_with_automatic_tbus(const char* a_pszTbusdUrl, int a_iFlag,
                                      int a_iTimeout);
 ...
 *@see
 */
int tconnapi_decode(IN const char* pszBuff, IN int iBuff,
                    OUT TFRAMEHEAD* pstHead, OUT int* piHeadLen);
int tconnapi_wap_decode(IN const char* pszBuff, IN int iBuff,
                        OUT TWAPUPLOAD2* pPkgBuff, INOUT int* piPkgLen);
 ...
 *@see
 */
int tconnapi_encode(IN char* pszBuff, INOUT int* piBuff,
                    IN TFRAMEHEAD* pstHead);
int tconnapi_wap_encode(OUT char* pszBuff, INOUT int* piBuff,
                        IN TWAPDOWNLOAD2* pPkgBuff, IN int iPkgLen);
...
/**
 *@brief
 根据对方BUS地址独占特定的通道(在调用tconnapi_connect之前，请先调用tconnapi_set_handle_opt)
 ...
 *@retval    =-2
 调用该接口的次数超过允许部署的tconnd的上限，目前只支持前端最多部署4个tconnd
 *@retval    =-3 输入参数非法

 *@note
 对于需要前端接入多个tconnd的情况，必须多次调用本接口，并且分别传入tconnd的bus地址
 */
 ...
 *       例如： iIdx = GET_UNMASK_IDX(iConnIdx)
 */
int tconnapi_recv(IN TCONNDHANDLE iHandle, INOUT int* piSrc, OUT char* pszBuff,
                  INOUT int* piBuff, OUT TFRAMEHEAD* pstHead);

/**
 *@brief 往bus中发包
 *@brief
 *在使用多对一部署时，iDst参数会无效，tconnd会根据framehead中的iConnIdx算出对端地址
 *@brief
 *如果希望使用iDst参数，将链接无关的消息发给特定tconnd，请将iConnIdx置为-1
 ...
 */
int tconnapi_send(IN TCONNDHANDLE iHandle, IN int iDst, IN char* pszBuff,
                  IN int iBuff, IN TFRAMEHEAD* pstHead);
...
/**
 *@brief
 *设置bus句柄属性，需要注意的是该api应该在tconnapi_create之后、tconnapi_connect之前调用
 *@brief 示例  int flag = 1; tconnapi_set_handle_opt(iHandle,
 *TCONNAPI_OPT_NAME_TBUS_EXCLUSIVE_CHANNELS ,&flag,sizeof(flag))
 *@param iHandle  				句柄指针
 ...
 */
int tconnapi_set_handle_opt(IN TCONNDHANDLE iHandle, IN int iOptionName,
                            IN const void* pvOptionValue,
                            IN unsigned int dwOptionLen);
 ...
 *@see
 */
int tconnapi_visualize_framehead(TFRAMEHEAD* pstHead, char* pszBuff,
                                 size_t* piBuff);
 ...
 */
void tconnapi_set_send_with_timestamp_flag(IN TCONNDHANDLE iHandle,
                                           IN int iFlag);

/**
 *@brief 获取上一个消息从进入 tbus 通道到当前时间的时间间隔(单位：ms)
 *@param iHandle[IN]  句柄指针
 *@param pllMsTime[OUT]     上一个消息从进入 tbus
 *通道到当前时间的时间间隔(单位：ms)
 *@param pstCurrentTime[IN]  	当前系统的时间，如果不为NULL，api
 *内部会使用此参数作为当前时间； 如果为NULL，api 内部会调用 gettimeofday
 *获取当前时间
 *
 *@retval   =0:success
 *@retval   =-1 输入参数非法
 ...
 *@retval   =-4 其他错误
 */
int tconnapi_get_lastmsg_timestamp(IN TCONNDHANDLE iHandle,
                                   OUT long long* pllMsTime,
                                   IN struct timeval* pstCurrentTime);
/** @} */

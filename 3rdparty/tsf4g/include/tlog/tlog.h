 ...
 * @param[in] a_pstctx 日志上下文
 * @return  -1 失败
 *			>=0 日志配置个数[tlog配置文件中CategoryList的个数]
 */
int tlog_get_category_size(LPTLOGCTX a_pstCtx);
 ...
 *			非null 获取的日志类指针
 */
LPTLOGCATEGORYINST tlog_get_category_by_index(LPTLOGCTX a_pstCtx, int index);

/**
 * 获取本地日志文件格式
 * @param[in] a_pstctx 日志类指针
 * @return  null 失败
 *			非null 日志文件pattern
 */
const char *tlog_get_category_pattern(LPTLOGCATEGORYINST a_pstCatInst);
 ...
 *          非NULL 获取的日志类指针
 */
LPTLOGCATEGORYINST tlog_get_bill_category(LPTLOGCTX a_pstCtx, const char *a_pszName);

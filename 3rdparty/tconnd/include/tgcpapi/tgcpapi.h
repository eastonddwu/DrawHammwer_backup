...
#define TGCP_MAX_SERVICE_NAME_LEN (128)
...
typedef enum tagAuthType {
  ...
  TGCP_AUTH_INTL = 0x1015,    // intl鉴权方式
  TGCP_AUTH_JWT = 0x1016,     // JWT (Json Web Token)

  TGCP_AUTH_VER_2 = 0x7fff,  // 新的帐号鉴权方式，2014.03.07
} eAuthType;

typedef enum TransType {
  TGCP_TRANS_TYPE_TP = 0x0001,
} TransType;
...
typedef enum tagAccountType {
  ...
  TGCP_ACCOUNT_TYPE_APPLE_OPENID,        // apple openid
  ...
  TGCP_ACCOUNT_TYPE_JWT_SUBJECT = 0x8012,
} eAccountType;
...
struct tagTGCPAccount {
  ...
  tagTGCPAccount() : uType(0), bFormat(TGCP_ACCOUNT_FORMAT_QQ_32), llUid(0) {}
};
...
typedef enum {
  ...
  TGCP_ROUTE_SPECIFY_SERVICE,    // 指定集群服务登录方式1
  TGCP_ROUTE_SPECIFY_NAME_DATA,  // 指定集群服务登录方式2
} eTGCPRouteType;
...
typedef struct {
  char service_name[TGCP_MAX_SERVICE_NAME_LEN];
} TGCPSPECIFYSERVICE, *LPTGCPSPECIFYSERVICE;

typedef struct {
  char service_name[TGCP_MAX_SERVICE_NAME_LEN];
  char user_data[TGCP_MAX_SERVICE_NAME_LEN];
} TGCPSPECIFYNAMEDATA, *LPTGCPSPECIFYNAMEDATA;
...
typedef union {
  ...
  TGCPSPECIFYSERVICE service;
  TGCPSPECIFYNAMEDATA namedata;
...
int tgcpapi_init(HTGCPAPI a_pHandle, int a_iServiceID, ePlatformID a_iPlatformID, unsigned short a_iAuthType,
                 eEncryptMethod a_iEncMethod, eKeyMaking a_iKeyMode, int a_iBuffLen);

// 设置客户端已开启哪些第三方组件
//  tgcpapi_set_transtype(h, TGCP_TRANS_TYPE_TP | TGCP_TRANS_TYPE_TSS);
void tgcpapi_set_transtype(HTGCPAPI handle, unsigned int transtype_mask);
...
/// @brief  获取第三方组件头部透传信息
/// @param[in]  a_pHandle       tgcpapi句柄
/// @param[out]  ext_data    buffer指针
/// @param[out]  ext_data_len    数据长度
///
/// @return =0, success
///         !0, fail, 调用tgcpapi_error_string()获取错误原因
int tgcpapi_get_transdata_ext(HTGCPAPI a_pHandle, int* trans_type, const char** ext_data, int* ext_data_len);

/// @brief  发送插件消息
/// @param[in]  a_pHandle       tgcpapi句柄
/// @param[out]  ext_data    填充到头部的内容，最大允许64字节
/// @param[out]  ext_data_len    数据长度
///
/// @return =0, success
///         !0, fail, 调用tgcpapi_error_string()获取错误原因
int tgcpapi_send_transdata_msg(HTGCPAPI a_pHandle, unsigned int trans_type, const char* ext_data, int ext_data_len,
                               const char* data, int data_len, int timeout);
...
/// @see  tgcpapi_query_wait_notify()
///
int tgcpapi_jump_connection(HTGCPAPI a_pHandle, const char* a_pszUrl, int a_iTimeout, const LPTGCPJUMP a_pExtData,
                            const tgcp_ulonglong* a_pServerId);
///  @}
...
/// @note 当返回TGCP_ERR_PEER_CLOSED_CONNECTION或TGCP_ERR_NETWORK_EXCEPTION时，仍然需要通过（iEvents &
/// TGCP_EVENT_DATAIN）来判断是否可读
///
/// @see  tgcpapi_init()
/// @see  tgcpapi_set_appid()
/// @see  tgcpapi_set_token()
...
///
/// @note 在(events & TGCP_EVENT_SSTOPED)为true时调用
int tgcpapi_get_sstop_ip_port_id(HTGCPAPI a_pHandle, char* a_pszTconndIp, int& a_iTconndIpSize, char* a_pszTconndId,
                                 int& a_iTconndIdSize, unsigned short& a_wTconndPort);
...
///
/// @see tgcpapi_flush
int tgcpapi_send(HTGCPAPI a_pHandle, const char* a_pszBuffIn, int a_iSize, int a_iTimeout, bool allow_lost = false);
...
/// @note
/// 返回值为TGCP_HAVE_RECV_GCP_INNER_MSG时，表明收到gcp内部消息，已经在内部处理掉，但是后续可能还有业务包，要继续尝试接受
...
int tgcpapi_query_wait_notify(HTGCPAPI a_pHandle, int* a_piBingo, LPQUEUENOTIFY a_pstQueNotify, int a_iTimeout);
///  @}
...
/// @note
/// 说明：GCP默认会使用当前TDR协议的最高版本号，在调用该接口设置版本号后，系统将使用设置的版本号进行TDR协议的打解包
///
...
/// @brief  设置trace数据
int tgcpapi_set_trace_data(HTGCPAPI a_pHandle, const char* a_pszBuffIn, int a_iSize);
